...
class User:
    ...
    def create(username, password):
        ...
        if not re.match(r"^[a-zA-Z0-9_一-鿿]{2,50}$", username):
...
class Email:
    ...
    def delete_all(user_id=None):
        if user_id:
            # 先拿到该用户所有 email_id
            rows = db.fetch_all("SELECT id FROM emails WHERE user_id = %s", (user_id,))
            email_ids = [r["id"] for r in rows]
            if email_ids:
                placeholders = ",".join(["%s"] * len(email_ids))
                db.execute(f"DELETE FROM classifications WHERE email_id IN ({placeholders})", email_ids).close()
                db.execute(f"DELETE FROM final_results WHERE email_id IN ({placeholders})", email_ids).close()
                db.execute(f"DELETE FROM paxos_logs WHERE email_id IN ({placeholders})", email_ids).close()
                db.execute(f"DELETE FROM emails WHERE id IN ({placeholders})", email_ids).close()
        else:
            db.execute("DELETE FROM classifications").close()
            db.execute("DELETE FROM final_results").close()
            db.execute("DELETE FROM paxos_logs").close()
            db.execute("DELETE FROM emails").close()
        return True
