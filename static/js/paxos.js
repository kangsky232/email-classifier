const PaxosPage = {
    ...
    load() {
        ...
        page.innerHTML = `
            <div class="page-header">
                <div>
                    <h2>${t("nav.paxos")}</h2>
                    <p class="page-subtitle">${t("classify.paxos_process")}</p>
                </div>
                <button class="btn" type="button" onclick="PaxosPage.fetchLogs()">${t("common.refresh")}</button>
            </div>
            <div class="card">
                <div class="toolbar">
                    <input id="paxos-email-id" class="form-control" style="max-width:180px" placeholder="Email ID" value="${App.escape(this.emailId)}">
                    <button class="btn" type="button" onclick="PaxosPage.applyEmailFilter()">${t("common.filter")}</button>
                    <button class="btn" type="button" onclick="PaxosPage.clearEmailFilter()">${t("common.reset")}</button>
                    <button class="btn btn-primary" type="button" onclick="PaxosPage.runDemo()">${t("classify.paxos_process")}</button>
                </div>
    ...
    renderLogs(result) {
        ...
        if (!rows.length) {
            box.innerHTML = App.empty(t("classify.no_paxos_logs"));
            return;
        }
        box.innerHTML = `
            <div class="table-wrap">
                <table>
                    <thead><tr><th>${t("email.created_at")}</th><th>Email ID</th><th>Proposal ID</th><th>${t("classify.method")}</th><th>Proposer</th><th>Value</th><th>Result</th></tr></thead>
                    <tbody>${rows.map((log) => `
                        <tr>
        ...
        try {
            ...
            App.showModal(`
                <div class="modal-header">
                    <h3>${t("classify.paxos_process")} #${emailId}</h3>
                ...
                `).join("")}</div>` : App.empty(t("classify.no_paxos_logs"))}
            `);
        } catch (error) {
            App.showToast(error.message, "error");
        }
        ...
        try {
            ...
            box.innerHTML = `
                <div class="demo-panel">
                    <strong>${t("classify.paxos_process")}</strong>
