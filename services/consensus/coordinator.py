from services.consensus.proposer import Proposer
from services.consensus.acceptor import Acceptor
from services.consensus.learner import Learner
from services.consensus.message import Message
from infrastructure.database.models import PaxosLog
import requests
import time
import logging
...
class PaxosCoordinator:
    ...
    def _http_post_with_retry(self, url, data, timeout=5, retries=1):
        for attempt in range(retries + 1):
            try:
                ...
                if attempt < retries:
                    logger.warning(f"Timeout calling {url}, retrying ({attempt+1}/{retries})")
                    continue
                raise
    ...
    def run_consensus(self, value):
        ...
        logger.info(f"Starting Paxos consensus for value: {value}")
        ...
        if len(promises) < majority:
            logger.warning(f"Prepare phase failed: {len(promises)}/{self.num_acceptors} promises received")
        ...
        if len(accepteds) < majority:
            logger.warning(f"Accept phase failed: {len(accepteds)}/{self.num_acceptors} accepts received")
        ...
        logger.info(f"Consensus reached! Final value: {final_value}, elapsed: {elapsed}ms")
    ...
    def _http_prepare_phase(self, prepare_msg):
        ...
        for url in self.acceptor_urls:
            try:
                resp = self._http_post_with_retry(
                    f"{url}/paxos/prepare",
                    {
                        "proposal_id": prepare_msg.proposal_id,
                ...
                logger.error(f"Acceptor [{url}] unreachable: {e}")
